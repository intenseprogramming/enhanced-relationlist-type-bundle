(function () {
    ...
    document.querySelectorAll(SELECTOR_CONTAINER).forEach(definitionContainer => {
        ...
        const headContainer = definitionContainer.querySelector(SELECTOR_HEAD_CONTAINER);
        const groupTemplate = JSON.parse(rootTable.getAttribute('data-group-template'));
        ...
        const addGroup = (event) => {
            ...
            const markup = `${groupTemplate.replace(/%identifier%/g, identifier)}`;
        ...
        headContainer.querySelector(SELECTOR_ADD_BUTTON).addEventListener('click', addGroup);
        addRowEventListeners();
        updateRemoveButton();

        definitionContainer.querySelector(SELECTOR_REMOVE_BUTTON).addEventListener('click', removeGroup);
