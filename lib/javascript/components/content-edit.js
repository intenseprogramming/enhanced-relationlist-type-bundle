...
export default class ContentEditBase extends Component {
    constructor(props) {
        ...
        let index = 0;
        this.state = {
            value: this.props.value.map((row) => {
                const item = {
                    value: row,
                    originalIndex: index++,
                };
        ...
        // empty lists are usually content/create so we can just add the system groups.
        if (this.props.value.length === 0) {
            const staticGroups = this.props.settings.groupSettings.groups;

            for (let key in staticGroups) {
    ...
    render() {
        return (
            <Table
                ...
                udwConfig={this.props.udwConfig}
...
ContentEditBase.propTypes = {
    ...
    udwConfig: PropTypes.object.isRequired,
