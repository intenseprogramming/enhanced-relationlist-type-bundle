...
export default class Selection extends BaseAttributeComponent {
    constructor(props) {
        ...
        if (this.state.value.settings.options.forEach) {
            const options = {};
            this.state.value.settings.options.forEach((item, index) => {
                options['' + index] = item;
                nextIndex = index;
            });
            nextIndex++;
            this.state.value.settings.options = options;
        } else {
            for (let index in this.state.value.settings.options) {
                if (this.state.value.settings.options.hasOwnProperty(index) && index >= nextIndex) {
        ...
        this.state.value.settings.options['' + (nextIndex++)] = {};
    ...
    componentDidUpdate(nextProps, nextState) {
        if (this.state && nextState && this.state.value !== nextState.value) {
            ...
            delete state.value.settings.options['' + (state.nextIndex - 1)];

            this.props.updateAttribute(state.value);
        }
    }
    ...
    toggleMultiple() {
        const state = _.cloneDeep(this.state);
        state.value.settings.multiple = !state.value.settings.multiple;
        this.setState(state);
    }
    ...
    removeOption(event, index) {
        ...
        const state = _.cloneDeep(this.state);
        delete state.value.settings.options[index];
        this.setState(state);
    }

    updateOption(event, index) {
        const state = _.cloneDeep(this.state);
        state.value.settings.options[index][this.props.language] = event.target.value;
        this.setState(state);
    }
    ...
    activateOption(event, index) {
        const state = _.cloneDeep(this.state);
        state.value.settings.options[index][this.props.language] = event.target.value;
        state.value.settings.options['' + (state.nextIndex++)] = {};
        this.setState(state);
    }
    ...
    renderOptions() {
        ...
        _.forEach(this.state.value.settings.options, (option, index) => {
            options.push(
            ...
            <Fragment>
                <div className="form-check form-check-inline">
                    <label className="checkbox-inline form-check-label">
                        <input className="form-check-input" type="checkbox" checked={this.state.value.settings.multiple} onChange={this.toggleMultiple.bind(this)} />
